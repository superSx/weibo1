						...
						<div class="MIB_mblogbgl">
							...
							<div class="mainR MIB_200 MIB_txtar MIB_linkar">
								...
								<div class="right_nav">
									...
									<ul>
										<li><a href="atMeList.action?pageNumber=1">@提到我的</a></li>
										<li><a href="commentsList.action?pageNumber=1&listType=1">我的评论</a></li>
										<li id="li_myprofile" class="cur MIB_txtar">我的收藏</a></li>
									</ul>
								</div>
								<div id="ads_35"></div>
								...
								<div class="MIB_mbloglist">
									<div class="newFilter MIB_txtal" id="MIB_newFilter" >
										<div  class="nfTagB nfTagOff">										
											<ul><li class="current"><span _link="">我的收藏</span></ul>
									...
									<div style="color: #B8B7B7;padding: 30px 5px;">
										<p class="title">亲，你还没有收藏微博哦。赶紧去收藏有趣的微博吧！</p>
									</div>
									</c:if>
									...
									<ul class="MIB_feed" id="feed_list" isWrite="1">
										...
										<li class="MIB_linedot_l" id="favor_${profId}">
											<div class="head_pic">
												<a namecard="true" href="/Weibo/Others/othersProfile.action?otherId=${prof.userinfo.userId}" ><img title="${prof.publishName}" src="${prof.publishFace}" /></a>
											</div>
											<div class="MIB_feed_c">
												<p>收藏于：${prof.collectTime}</p>
												<p class="sms" type="1"><a href="/Weibo/Others/othersProfile.action?otherId=${prof.userinfo.userId}" title="${prof.publishName}">${prof.publishName}</a>：${prof.publishContent}
												<!-- 转发微博内容 -->
							  						...
							  						<c:choose>
														...
														<div class="feed_att MIB_linkbl MIB_txtbl">
															<div class="lf">
																...
																<strong>来自</strong><cite><a href="" target="_blank"  onclick="/Weibo/Others/othersProfile.action?otherId=${prof.tsmit.userinfo.userId}">${prof.tsmit.publishName}</a></cite>
															</div>
															<div class="rt">
																<!-- 转发 -->
												...
												<div class="feed_att MIB_linkbl MIB_txtbl">					
														<div class="lf">
															...
															<strong lang="CL1006">来自</strong><cite><a href="/Weibo/Others/othersProfile.action?otherId=${prof.userinfo.userId}" target="_blank">${prof.publishName}</a></cite>
														</div>
														<div class="rt">
															<!-- 转发 -->
															...
															<!-- /转发 -->
															<span class="MIB_line_l">|</span>
															<a href="javascript:void(0);" onclick="onClickDelete('${prof.favorId}');"><strong lang="CL1003">取消收藏</strong></a>
															<!-- 评论 -->
															<span class="MIB_line_l">|</span>
															<a id="" href="javascript:void(0);" onclick="onClickComment('comment_${prof.profId}')" >
												...
												<div class="MIB_assign rt" popcontainer="true" id="comment_${prof.profId}" style="display:none" > 
													...
													<div class="MIB_assign_c MIB_txtbl">
														...
														<div class="logininput new_position">
															...
															<a class="btn_normal" href="javascript:onClickSubmitComment('input_${prof.profId}');"><em>评论</em></a>
									...
									<div class="MIB_assign_b"></div>
											...
											<div class="clear"></div>
											<div class="MIB_assign" id="" style="display:none;" ></div>
										</li>
										</c:forEach>
									</ul>
									<!--/列表-->
									...
									<div class="MIB_bobar">
										...
										<div class="fanye MIB_txtbl" style="float:right;" id="page">
										    <a href="collectionList.action?pageNumber=1" class="btn_num btn_numWidth"><em>首页</em></a>
										    <c:if test="${pageNumber>1}">  
										        <a href="collectionList.action?pageNumber=${pageNumber-1}" class="btn_num btn_numWidth"><em>上一页</em></a>
										    </c:if>     
										    <c:forEach begin="1" end="5" step="1" var="pageIndex">  
				 								<c:if test="${pageNumber-(6-pageIndex) >=1}">  
										            <a class="btn_num"  href="collectionList.action?pageNumber=${pageNumber+pageIndex-6}"><em>${pageNumber+pageIndex-6}</em></a>
										        </c:if>  
											</c:forEach>
											<span>${pageNumber}</span>
											<c:forEach begin="1" end="5" step="1" var="pageIndex">  
				 								<c:if test="${pageNumber+pageIndex <= pageBean.totalPages}">  
										            <a class="btn_num"  href="collectionList.action?pageNumber=${pageNumber+pageIndex}"><em>${pageNumber+pageIndex}</em></a>
										        </c:if>  
											</c:forEach>  
										    <c:if test="${pageNumber < pageBean.totalPages}">  
										        <a href="collectionList.action?pageNumber=${pageNumber+1}" class="btn_num btn_numWidth"><em>下一页</em></a>
										    </c:if>    
										    <a href="collectionList.action?pageNumber=${pageBean.totalPages}" class="btn_num btn_numWidth"><em>末页</em></a>
										</div>
										</c:if>
									</div>
									<!--翻页-->
