   ...
   <div class="MIB_blogb">
    ...
    <div id="atme_filter" class="atme_filter1">									
		<div act="panel" class="atmef_con1 clearFix">
			<ul class="lf">
			<li>
               <a href="frommsgList.action?pageNumber=1">已收到的私信</a>
            </li>
     ...
     <!-- 直接加 -->
	   <c:forEach items="${msgList}" var="msg">
		<li id="${msg.msgId }" class="MIB_linedot_l" onmouseover="mouseovershowDialog('messages_cancle1_${msg.msgId}')" onmouseout="mouseovercloseDialog('messages_cancle1_${msg.msgId}')" style="z-index:inherit;">
		 <div class="user_pic user_Stat">
		  <div class="head_pic"><a href="" title=""><img title="" alt="" imgtype="head" src="${sessionScope.user.myFace}" width="50" height="50"></a>
		  </div>
		 </div>
         <div class="MIB_feed_c">
		  <div class="rt icon_closel" title="删除" id="messages_cancle1_${msg.msgId}" style="display:none;"><a href="javascript:void(0);" onclick="OnClickremove(${msg.msgId })" >x</a>
		  </div>
          <p class="letter">
		  我发给&nbsp;<a namecard="true" id="msg_${msg.msgId}" href="/Weibo/Others/othersProfile.action?otherId=${msg.msgTo}" title="${msg.msgTo} ">@${msg.msgToName}</a>&nbsp;的私信：&nbsp;&nbsp; ${msg.msgContent}  
		  </p>
          <div class="feed_att MIB_linkbl">
		   <div class="lf">
            <span class="gray9">${msg.sendTime }
            </span>
            <em class ="reportBtn">
            </em>
           </div>
           <div class="rt">
           	<input type="hidden" id="msgToName_${msg.msgId }" value="${msg.msgToName}"/>
									...
									<div class="MIB_bobar">
										...
										<div class="fanye MIB_txtbl" style="float:right;" id="page">
										    <a href="msgList.action?pageNumber=1" class="btn_num btn_numWidth"><em>首页</em></a>
										    <c:if test="${pageNumber>1}">  
										        <a href="msgList.action?pageNumber=${pageNumber-1}" class="btn_num btn_numWidth"><em>上一页</em></a>
										    </c:if>     
										    <c:forEach begin="1" end="5" step="1" var="pageIndex">  
				 								<c:if test="${pageNumber-(6-pageIndex) >=1}">  
										            <a class="btn_num"  href="msgList.action?pageNumber=${pageNumber+pageIndex-6}"><em>${pageNumber+pageIndex-6}</em></a>
										        </c:if>  
											</c:forEach>
											<span>${pageNumber}</span>
											<c:forEach begin="1" end="5" step="1" var="pageIndex">  
				 								<c:if test="${pageNumber+pageIndex <= pageBean.totalPages}">  
										            <a class="btn_num"  href="msgList.action?pageNumber=${pageNumber+pageIndex}"><em>${pageNumber+pageIndex}</em></a>
										        </c:if>  
											</c:forEach>  
										    <c:if test="${pageNumber < pageBean.totalPages}">  
										        <a href="msgList.action?pageNumber=${pageNumber+1}" class="btn_num btn_numWidth"><em>下一页</em></a>
										    </c:if>    
										    <a href="msgList.action?pageNumber=${pageBean.totalPages}" class="btn_num btn_numWidth"><em>末页</em></a>
										</div>
										</c:if>
									</div>
									<!--翻页-->
