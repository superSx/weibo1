			        ...
			        <div class="mainR MIB_200 MIB_txtar MIB_linkar">
						...
						<div></div>
                         ...
                         <div class="btmTit_wrap csTitle">
                           	<div class="lf">
                           		<c:choose>
									<c:when test="${other.sex == 1}">他关注了${other.followCount}人</c:when>
									<c:otherwise>她关注了${other.followCount}人</c:otherwise>									
								</c:choose>
                            </div>
                            <div class="rt">
                            	<label>
                        ...
                        <!--/评论区 -->
									...
									<ul class="concernBox" id="att_wrap" >
										...
										<c:if test="${followPageBean.totalPosts == 0}">
										<span>他/她还没有关注别人呦</span>
										</c:if>
										<c:forEach items="${followList}" var="follow">
									  		<li class="MIB_linedot_l" style="z-index:inherit;">
									    		<div class="conBox_l"><a href="/Weibo/Others/othersProfile.action?otherId=${follow.followId}"><img title="${follow.followName }"  src="${follow.fmyface }" /></a>
												</div>
									  			<div class="conBox_c" style="z-index:inherit;"> 
									  				<span class="name"> 
									  					<a href="/Weibo/Others/othersProfile.action?otherId=${follow.followId}" title="${follow.followName }">${follow.followName }</a> 
									  				</span>
									    ...
									    <div id="follow_conBox_${follow.fmId}" class="conBox_r" style="display:block;">
									      <c:if test="${follow.followId == sessionScope.user.userId }">
									      </c:if>
									      <c:if test="${follow.othersfollowack}">
									      	<p class="mutual">
												<a class="concernBtn_Yet"><span class="add_yet"></span>已关注</a>
											</p>
									      </c:if>
									      <c:if test="${!follow.othersfollowack && follow.followId != sessionScope.user.userId}">
									      	<p  id="collect_${follow.followId }">
												<a onclick="setOthersFansId(${follow.followId});"  class="addFollow">
							...
							<div class="fanye MIB_txtbl" style="float:right;" id="page">
							    <a href="/Weibo/Others/othersFollow.action?pageNumber=1 &otherId=${other.userId }" class="btn_num btn_numWidth"><em>首页</em></a>
							    <c:if test="${pageNumber>1}">  
							        <a href="/Weibo/Others/othersFollow.action?pageNumber=${pageNumber-1}&otherId=${other.userId }" class="btn_num btn_numWidth"><em>上一页</em></a>
							    </c:if>     
							    <c:forEach begin="1" end="5" step="1" var="pageIndex">  
	 								<c:if test="${pageNumber-(6-pageIndex) >=1}">   
							            <a class="btn_num" href="/Weibo/Others/othersFollow.action?pageNumber=${pageNumber+pageIndex-6}&otherId=${other.userId }"><em>${pageNumber+pageIndex-6}</em></a>
							        </c:if>  
								</c:forEach>
								<span>${pageNumber}</span>
								<c:forEach begin="1" end="5" step="1" var="pageIndex">  
	 								<c:if test="${pageNumber+pageIndex <= followPageBean.totalPages}">  
							            <a class="btn_num"  href="/Weibo/Others/othersFollow.action?pageNumber=${pageNumber+pageIndex}&otherId=${other.userId }"><em>${pageNumber+pageIndex}</em></a>
							        </c:if>  
								</c:forEach>  
							    <c:if test="${pageNumber < followPageBean.totalPages}">  
							        <a  href="/Weibo/Others/othersFollow.action?pageNumber=${pageNumber+1}&otherId=${other.userId }" class="btn_num btn_numWidth"><em>下一页</em></a>
							    </c:if>    
							    <a  href="/Weibo/Others/othersFollow.action?pageNumber=${followPageBean.totalPages}&otherId=${other.userId }" class="btn_num btn_numWidth"><em>末页</em></a>
							</div>
							</c:if>
							</div>
							<!--翻页-->
